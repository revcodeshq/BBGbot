const { SlashCommandBuilder, EmbedBuilder, PermissionFlagsBits } = require("discord.js"); // Added EmbedBuilder
 ...
 */
async function checkPlayerId(fid) {
  const params = {
    fid: String(fid),
 ...
 */
async function getCaptchaImage(fid) {
  const genParams = { 
    fid: String(fid),
    time: String(Date.now()) 
  };
 ...
 */
async function redeemGiftCode(fid, code, captcha) {
  const params = {
    fid: String(fid),
    cdk: String(code),
...
module.exports = {
  ...
  async execute(interaction) {
    ...
    for (const [index, user] of usersToRedeem.entries()) {
        ...
        const result = await redeemGiftCode(fid, code, solvedCaptcha);
